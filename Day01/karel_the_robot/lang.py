...
class Lang :
    ...
    def consume(self, what) :
        ...
        else : self.exit ("Expecting %s" % what)
    
    def exit(self, mesg) :
        import sys
        trace = "<%s>" % " ".join(self.ahead)
        if mesg : print ("Fatal error: %s %s" % (mesg,trace))
        sys.exit()

    def execInstruction(self, inst) :
        if not inst : return
        if   inst == "move"       : self.env.move()
        elif inst == "turnleft"   : self.env.turnleft()
        elif inst == "pickbeeper" : self.env.pickbeeper()
        elif inst == "putbeeper"  : self.env.putbeeper()
        elif inst == "turnoff"    : self.exit(None)
        elif inst == ""           : pass
            ...
            for subInst in inst : self.execInstruction(subInst)

        else : self.exit("Illegal instruction: %s" % inst)

    def getword(self) :
        if not self.words : return "EOF"
        self.ahead = self.words[:6]     # for error messages
    ...
    def nextword(self) : 
        if self.words : return self.words[0]
        else          : return None
