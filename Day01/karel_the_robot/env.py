...
#       self.board = filter(lambda x: len(x)>0, self.board)
        self.board = [x for x in self.board if len(x)>0]
...
#        print('BEEPERS:', self.beepers)
    ...
    def move(self) :
        ...
        if not self.isBlocked(dest) :
            ...
            self.karelPos = dest
            if beeps : old = str(beeps)[0]  # just room for 1 digit
            else     : old = "."
            self.boardPut(kpos,old)         # replace prev content
        else : self.fatal("move is blocked")
        self.printBoard()

    def pickbeeper(self) :
        kpos = self.karelPos
        if not self.beepers.get(kpos) : self.fatal("No beeper to pick up")
        else :
            self.beepers[kpos] -= 1
            self.karelBeepers  += 1
        self.printBoard()
    ...
    def putbeeper(self) :
        kpos = self.karelPos
        if not self.karelBeepers : self.fatal("No beeper to put down")
        else :
    ...
    def test(self, cond) :
        return conditions[cond](self)  # test the condition

    def printBoard(self) :
        time.sleep(self.interval)
        os.system('cls')   # in windows change 'clear' to 'cls'
        for row in range(self.nrows) :
            print ("".join(self.board[row]))  # make a string from list
...
conditions = { 
  ...
  "next-to-a-beeper"     : lambda env:     env.beepers.get(env.karelPos),
  "not-next-to-a-beeper" : lambda env: not env.beepers.get(env.karelPos),
